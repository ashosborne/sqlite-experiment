...
- Generated: 2026-08-14T12:45:20Z
- App status: `in_progress` · completeness: `incomplete`
- Manifest last_updated: 2026-08-14T12:45:19Z by `sqlite-engine-v50-openwrite`
...
| full (converted) | 240 | Behaviour done in modern; parity may still be UNVERIFIED |
...
- `btree-002` — confidence=observed-in-code; Cursor positioning and mutation incl. page balancing; file-format invariants. run-56: NONE -> PARTIAL - file-backed INSERT / DELETE / literal UPDATE of a plain single-leaf rowid table move CELLS through a table cursor on the pager's leaf pages (schema-rootpage walk, read_leaf/write_leaf cell packing, file change-counter bump), NOT a whole-image rebuild; a cursor-op counter moves on the file DML and NOT on a :memory: control; the committed file stays valid SQLite (the pinned C amalgamation opens a cursor-written file, reads the runtime row, integrity_check rc 0); reopen reparses the cursor-written leaf. Pinned engine-btree45-002. run-57: FIRST SPLIT landed ON THE CURSOR PATH - when cells no longer fit one 0x0d leaf the cursor path itself produces a table interior root (0x05, C's divider-cell layout) plus >=2 leaf 0x0d pages on appended pages (page_count 2->4 matching C), a split counter proving no dbfile whole-image fallback took the overflow INSERT; post-split writes stay on the cursor path (interior read lifted, re-split); the PINNED C AMALGAMATION is compiled and exec'd against modern-written split files (every row incl. a runtime payload, integrity_check ok - engine-btree46). RESIDUAL (full forbidden): no sibling-balance matrix (the split redistributes cells across leaves rather than balancing siblings in place), no merge on DELETE (shrink-below-split falls back to the whole-image writer), no 3-level trees, no index b-trees, no WITHOUT ROWID, no overflow-page chains on the cursor path, no saved-position restore matrix. run-59: LIVE READS of the v48 scan slice (plain SELECT col(s) FROM the single rowid table, non-IPK, autocommit, non-WAL) now walk btree cells via read_table_cells / cell payload decode on the VDBE cursor - not the in-memory store; a cursor-read counter proves the leaf (and split interior root) walk. run-60: the v49 WHERE-compare and rowid-seek shapes read cells through the same cursor (the compare is a VM opcode over cell-decoded registers; rejected rows are still positioned on). Kitchen still owns every other read shape: joins, AND/OR and non-equality-family predicates, expressions, aggregates, IPK tables, multi-table schemas, WAL mode, and all reads inside an open transaction. run-61: the VDBE Insert path now DRIVES the cursor write - OP_Insert's cell payload is put verbatim through insert_cell (the same leaf packing / split machinery, cursor-op counter ticks), not dbfile::write_db_bytes; kitchen DML still flushes through cursor_or_whole_image as before.
...
- `vdbe-engine-001` — confidence=observed-in-code; 199 OP_ cases; interruption, progress-callback and error unwinding semantics. | run-58 impl_in_modern=partial: the first bytecode slice. Constant SELECTs (SELECT n / n WHERE 1|0 / n+m / 'text' / n,m) compile to C's exact program (Init/Integer/String8/Add/ResultRow/Halt/Goto with C's p1-p5 and layout: WHERE-0 jumps Goto->Halt, 1+2 stays UNFOLDED with init-section loads after Halt), EXPLAIN returns that listing row-for-row against frozen C, and sqlite3_step of the same SQL executes the program through a real dispatch loop (register file, pc jumps, dispatch counter moves; kitchen-fallback SQL does not move it). run-59: FIRST TABLE-SCAN OPCODES landed - SELECT col(s) FROM one file-backed plain rowid table compiles to C's program (Init/OpenRead/Rewind/Column/ResultRow/Next/Halt/Transaction/Goto with C's p1-p5: OpenRead p2 = the REAL root page, p4 = column-count hint, Transaction p3 = the schema cookie read from the file header, Next p5=1), EXPLAIN matches frozen C row-for-row, and sqlite3_step walks btree cells through the dispatch loop - OP_Column decodes the CURRENT CELL'S payload parsed from the file image (read_table_cells), never the kitchen store; a cursor-read counter moves on the scan (incl. over a split interior root) and not on SELECT 1 or a kitchen join; runtime payloads round-trip through Column (engine-vdbe48 goldens). run-60: FIRST WHERE ON THE CURSOR - SELECT col(s) FROM t WHERE col <op> <int-lit|?> compiles to C's inverted jump-to-Next program (= emits Ne, <> Eq, > Le, < Ge, >= Lt, <= Gt, all p4=BINARY-8 p5=84, literal via init-section Integer, ? via Variable reading the 1-based statement params) and WHERE rowid=N compiles to C's Integer+SeekRowid single-cell shape; the compare runs AS AN OPCODE in the dispatch loop over btree cells (never a Rust filter - the cursor-read counter proves the scan positions on REJECTED rows too), and the result rows match frozen C incl. a runtime payload behind a decoy row (engine-vdbe49 goldens). RESIDUAL: AND/OR, LIKE/ GLOB/IS NULL/BETWEEN, text-literal compares, joins, expressions in the select list (length(b) still the pinned kitchen boundary), aggregates/ORDER BY, index OpenRead / SeekGE / IdxGE, OpenWrite / DML codegen, IPK tables (C emits Rowid - unimplemented), multi-table/attached/WAL scopes, 3-level trees, the other ~180 opcodes, OP_Program triggers, interrupt, progress handler, Transaction cookie tracking beyond the single-DDL frozen scope, SeekRowid as a page DESCENT (the seek positions over the parsed cell list, not a b-tree binary search) - all still the kitchen evaluator, honestly named. run-61: FIRST DML BYTECODE - single-row INSERT INTO t[(cols)] VALUES(lit|?, ...) on the v48 scope compiles to C's program (OpenWrite real root p4=k, value loads, NewRowid, MakeRecord p4=affinity string, Insert p4=table p5=57, WRITE Transaction p2=1) and sqlite3_step dispatches it: OP_Insert's cell (the MakeRecord-built payload, verbatim) goes through the pager cursor packing (insert_cell, split-capable) and is committed via the journal mini-txn; the store only mirrors the row afterwards; last_insert_rowid/changes/total_changes match C; VDBE-inserted rows read back through the v48/v49 cell scans incl. a reopened-file runtime payload (engine-vdbe50 goldens). DML RESIDUAL: UPDATE/DELETE bytecode, INSERT SELECT / multi-row VALUES / DEFAULT VALUES / upsert, constraint enforcement (NOT NULL/UNIQUE/CHECK/DEFAULT/FK tables stay kitchen), hooked/authorized connections (update/commit hooks, authorizer - gated to kitchen), the sqlite3_exec path (kitchen; only prepared statements ride the VM), MakeRecord affinity APPLICATION (the p4 string is replicated, not applied - vdbe-engine-002 stays none), IPK NewRowid.
...
| --- | --- |
| Surfaces total | 244 |
| Behaviours known | 359 |
| Seeds scanned | 109 |
| Unscanned hints (residual) | 3 |
| legacy_green flags | 270 |
