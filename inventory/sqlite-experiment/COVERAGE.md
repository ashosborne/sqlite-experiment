...
- Generated: 2026-08-11T20:25:48Z
- App status: `in_progress` · completeness: `incomplete`
- Manifest last_updated: 2026-08-11T20:25:18Z by `factory-operator-progress-scoreboard`
...
| partial | 75 | Some modern execution; gaps in notes |
| full (converted) | 11 | Behaviour done in modern; parity may still be UNVERIFIED |
...
- `builtin-scalar-agg-funcs-001` — upper/lower/length/substr/coalesce/typeof/hex/abs/quote/printf real; most of the ~60 core scalars absent
...
- `ddl-schema-001` — CREATE/DROP TABLE real (memory + durable file); views absent
...
- `dml-codegen-002` — OR IGNORE / OR REPLACE real; OR ROLLBACK/ABORT/FAIL and CHECK constraints absent
...
- `foreign-keys-002` — ON DELETE CASCADE real; SET NULL/SET DEFAULT/RESTRICT and ON UPDATE actions absent
...
- `misc-decimal-001` — decimal_add/decimal_cmp/decimal_mul exact scaled-integer real; decimal_sub/collation/pow2 absent
...
- `misc-rot13-001` — rot13() function real; rot13 collating sequence absent
...
- `name-resolution-001` — qualified (t.col) + bare resolution real in store/eval; ambiguity errors absent
...
- `pragma-surface-001` — 11 pragmas + integrity/quick_check ok-shape real; rest of the ~70-pragma dispatcher absent
...
- `printf-format-001` — printf %d/%s/%q/%Q/%% real; width/precision/%x/%f/%g and format() breadth absent
...
- `select-codegen-002` — UNION / UNION ALL with dedup real; INTERSECT/EXCEPT absent
...
- `triggers-001` — CREATE TRIGGER persisted (memory + durable); only AFTER INSERT triggers parse
- `triggers-002` — AFTER INSERT firing with new.col real; BEFORE/UPDATE/DELETE, old.*, WHEN clauses absent
...
- `date-time-funcs-001` — date-time-funcs — legacy_green yes
- `date-time-funcs-002` — date-time-funcs — legacy_green yes
- `date-time-funcs-003` — date-time-funcs — legacy_green yes
- `date-time-funcs-004` — date-time-funcs — legacy_green yes
...
- `misc-func-packs-001` — misc-func-packs — legacy_green yes
